...
class GeneInfo(object):
    def __init__(self, entrez_id):
        self.api_keys_path = 'api_keys.csv'
        self.mgi_db_path = './db/MGI.db'
        
        self.api_keys = {}
        if os.path.exists(self.api_keys_path):
            api_key_df = pd.read_csv(self.api_keys_path, header=None, index_col=0)
            self.api_keys = dict(zip(api_key_df.index.values, api_key_df[1]))
        else:
            print('api_keys.csv does not exist.')
    ...
    def fetch_basespace(self):
        ...
        if self.api_keys.get('basespace') is None:
            self.basespase_result = {param['param_name']: pd.DataFrame() for param in param_list}
            # print(self.basespase_result)
            return None
